...
static int32_t find_smallest_hole(uint32_t size, uint8_t page_align,
    ...
    uint32_t iterator = 0;
    while (iterator < (uint32_t)heap->index.size) {
        header_t *header =
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap) {
    ...
    if (iterator == -1) {
        ...
        iterator = 0;
        // Vars to hold the index of, and value of, the endmost header found so
        // far
        uint32_t idx = -1;
        uint32_t value = 0x0;
        while (iterator < (int32_t)heap->index.size) {
            uint32_t tmp =
              (uint32_t)lookup_ordered_array(iterator, &heap->index);
            if (tmp > value) {
                value = tmp;
                idx = iterator;
            }
    ...
    if (orig_hole_size - new_size < sizeof(header_t) + sizeof(footer_t)) {
        // Then just increase the requested size to the size of the hole we
        // found
...
void kfree(void *p, heap_t *heap) {
    ...
    if (test_header->magic == HEAP_MAGIC && test_header->is_hole) {
        ...
        test_footer =
          (footer_t *)((uint32_t)test_header + // Rewrite it's footer to point
                                               // to our header
